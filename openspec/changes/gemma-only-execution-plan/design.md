...
### Decision 7: Insert a Section 4 Mathematical Framework anchoring EECS 6699 lectures

**Choice:** Restructure the paper from 9 sections to 10 sections by inserting a new **§4 Mathematical Framework** between Related Work (§3) and the empirical sections (§5–§8). Merge old §6 (Abliteration) and §7 (Weight-Diff) into a combined §7 (Abliteration & Comparative Weight Diff). The M6 Rank-1 Cascade — already drafted as `paper/sections/08_rank1_cascade.md` — keeps its §8 slot. Discussion + Course Connections moves to §9; Conclusion + Ethics moves to §10. Strengthen the §9 course-connection requirement from "≥3 narrative connections" to "≥5 connections with at least one quantitative." Reference `docs/findings/course-material-mapping.md` as the canonical lecture-to-module map from both §4 and §9.

**Rationale:** After the in-person presentation, the EECS 6699 professor flagged that the project's content "feels too new and not visibly tied to lecture material," even though the underlying math (high-dim geometry, matrix perturbation, lazy/NTK) is exactly what Lectures 5–9 cover. The disconnect was rhetorical, not substantive: the existing draft sections (§2 Background, §3 Related Work, §8 Cascade) never name a lecture, never invoke a course theorem by name, and never compute a course-derived quantity. The fix is to insert a single dedicated section that invokes the relevant lecture results *before* the empirical sections cite them, plus two lightweight numeric measurements (Mirsky-style relative perturbation bound + activation-anisotropy figures) computable from existing on-disk artifacts (no new GPU runs).

**Math precision in §4 — singular values, not eigenvalues:** Because Gemma 4's `o_proj` and `down_proj` are rectangular matrices, the standard eigenvalue Hoffman-Wielandt inequality does not apply directly. §4 SHALL state the bound in singular-value form via **Mirsky's theorem** `Σᵢ (σᵢ(W+E) − σᵢ(W))² ≤ ‖E‖_F²`, and note that for unit `d` the perturbation `E = α·d·(dᵀW)` is rank-1 with `‖E‖_F = ‖E‖_2 = |α|·‖dᵀW‖_2`. The headline numeric in §4 is the **relative perturbation** `‖E‖_F / ‖W‖_F` per layer for two direction artifacts: D3 (the M6 winner, `refusal_directions_d3.pt`) and raw M2b (the negative baseline). This is the spec-mandated quantitative tie between §4 math and §8's 40.5% partial-effect finding.

**Lec 5 framing — directions, not activations:** A naive "near-orthogonality histogram of activations" misleads, because raw L15 activations are highly anisotropic. The anisotropy shows up as a **bulk mean shift** (empirical mean pairwise cosine ≈ 0.96, vs the Gaussian null mean of 0 for unit vectors in 2560-dim), not as inflated spread (empirical centered std ≈ 0.014, slightly *below* the isotropic null σ = 1/√2560 ≈ 0.020). Either way the raw activation cloud sits in a narrow cone, far from the Gaussian N(0, 1/d) null. The Lec 5 near-orthogonality result applies to *random* vectors, which approximate the geometry of *learned conceptual directions* (refusal direction, category means) — not the raw activation cloud. §4 therefore embeds three figures: projection-energy onto `d`, cosine matrix between learned category directions, and an honest activation-anisotropy figure with the Gaussian null overlaid as caveat. The load-bearing claim is near-orthogonality of *directions*, not isotropy of *points*.

**Lec 8–9 framing — plausibility, not prediction:** NTK / lazy training predicts the model's behavioral output is approximately a *linear* functional of a small rank-1 perturbation, not specifically a *partial* response. §9 prose ties Lectures 8–9 to §8's 40.5% partial result as "the lazy/linearization lens makes a partial first-order response plausible" or "consistent with a linear behavioral response," NOT "NTK predicts partiality." The residual ~60% refusal is evidence that the behavior is not fully one-dimensional, which the rank-1 ansatz does not capture.

**Alternative considered — run abliteration on Gemma 4 E2B for a width-scaling story:** rejected as default. The TA suggestion to "run on a model from lecture" reads literally as testing a smaller model, but the lectures introduce mathematical frameworks rather than named LLMs. An E2B sanity run could give a Lec 9 NTK-scaling angle but costs ~4 hours of GPU time and is downstream of the §4 framing change. Defer to a follow-up if time permits after the rest of M5 is complete.

**Alternative considered — strengthen §9 in place without adding §4:** rejected. Concentrating all lecture-anchoring in §9 leaves the empirical sections (§5–§8) still reading as "new content" that happens to be discussed against the course at the end. A dedicated §4 invokes the math up-front, so §5–§8 are then applications of the lecture machinery rather than freestanding empirics.

**What we lose:** one additional drafting cycle (§4 prose, two short numerics scripts, three figures, a section-reference audit pass over existing drafts). Estimated ~1 day of writeup work. No experimental work is redone; no GPU runs are added.
...
- **[All three diffs look very different]** → Also paper-worthy ("safety subspace is under-determined"); ties back to mechanistic Section 6 (Mechanistic Analysis under the 10-section outline; was §5 in the pre-Decision-7 numbering).
