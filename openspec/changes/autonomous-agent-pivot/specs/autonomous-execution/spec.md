...
- **THEN** it SHALL be launched with `cwd=../gb-mech/`, branch `agent/mechanistic-analysis` already checked out, and a prompt referencing only section 5 of `tasks.md`
...
- **WHEN** an agent is dispatched to execute "tasks.md section 6 (abliteration)"
...
- **M2 GPU experiments complete**: All tasks under sections "Benchmark Evaluation Pipeline", "Activation Extraction & Mechanistic Analysis", and "Abliteration Implementation & Experiments" in `alignment-geometry-study/tasks.md` are checked off on their respective `agent/*` branches, with figures in `results/figures/`.
- **M3 CPU weight diff complete**: All tasks under "Weight Diff & Cracked Model Analysis" are checked off on `agent/weight-diff`, with `results/weight_diffs/` populated.
...
- **THEN** an agent SHALL verify by running a checklist: every sub-task in sections 3, 4, 5 of the original tasks.md is `[x]`, every expected file in `results/` exists and is non-empty, and every required figure PNG exists in `results/figures/`, before answering yes
