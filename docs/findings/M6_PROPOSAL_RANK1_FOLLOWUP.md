...
**H5 (norm preservation) refuted empirically.** A full 42-layer × 2-projection-type audit of D3 vanilla projection vs base on `o_proj` and `down_proj` (computed in `scripts/m6_figures.py:figure_5_row_norms`, output `results/figures/m6_row_norm_distribution.png`) shows that vanilla rank-1 projection at α=1.0 changes per-row L2 norms by **mean 0.038 %, median 0.013 %, p99 0.34 %, max 9.73 %** across the 215 040 audited rows. The 9.73 % max is a single isolated outlier row at L01 o_proj; per-layer worst-row changes otherwise stay under 3 % across all 42 layers, and per-layer **mean** row-norm changes stay tightly in the **0.024–0.069 %** band — well below the 1 % threshold at which RMSNorm's downstream behaviour would plausibly drift. (The original §7.6 audit covered only 7 layers `[0, 5, 11, 15, 17, 23, 41]` and, by chance, did not include L01; on that 7-layer subset the worst-case is 2.83 % at L0 down_proj, reproducible. The full audit narrows the headline mean and exposes one isolated row outlier without changing the H5 conclusion.) Indeed the n=6 smoke is identical between D3-vanilla and 3a-biprojection on every prompt, including the same single residual refusal (pipe bomb) — the behavioural identity is the load-bearing evidence that H5 is refuted; the row-norm distribution is supporting context.
...
| H5 — norm-preserving biprojection is necessary on Gemma 4 | **refuted** | Stage 3a: 1/6 smoke identical per-prompt to D3 vanilla. Full 42-layer audit: vanilla projection changes per-row norms by mean 0.038 % / median 0.013 % / p99 0.34 % / max 9.73 % (isolated L01 outlier) — RMSNorm's behaviour does not depend on row-norm shifts at this magnitude, as confirmed by the per-prompt-identical D3 vs 3a outputs. |
